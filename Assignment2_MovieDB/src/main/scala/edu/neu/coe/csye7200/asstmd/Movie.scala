...
object Reviews {
  def apply(params: List[String]): Reviews = params match {
    //case imdbScore :: facebookLikes :: contentRating :: numUsersReview :: numUsersVoted :: numCriticReviews :: totalFacebookLikes :: Nil => apply(Try(imdbScore.toDouble), Try(facebookLikes.toInt), Rating.apply(contentRating), Try(numUsersReview.toInt), Try(numUsersVoted.toInt), Try(numCriticReviews.toInt), Try(totalFacebookLikes.toInt))
